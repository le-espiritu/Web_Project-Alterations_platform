...
<html>
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				$("#category").change(function(){
					location.href="/webproject/board/reviewlist.do?category="
																+encodeURI($(this).val())
				});
			});
		</script>
	...
	<body>
	<% ArrayList<BoardVO> reviewlist = (ArrayList<BoardVO>)request.getAttribute("reviewlist");
					...
					<div style="padding-top: 30px; width:95%" id="board">
						...
						<table class="table">
							...
							<tbody>
								 <%for(int i=0;i<size;i++){
									BoardVO board = reviewlist.get(i);
								%>
									<tr>
										<td><%=board.getBoard_no() %></td>
										<td><img src="<%=board.getReview_img() %>" alt="후기" class="photo"/>
										<td><a href="/webproject/board/reviewread.do?board_no=<%= board.getBoard_no()%>&state=READ"><%= board.getTitle() %></a></td>
						...
						<form action="/webproject/board/search.do">
							...
							<ul class="navbar-nav" style="float: right;">
								<li class="nav-item"><a href="/webproject/reviewinsertPage.do" style="text-align: right;">글쓰기</a></li>
							</ul>
